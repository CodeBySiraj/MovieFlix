    ...
    <main className="container mx-auto px-4 py-16 flex flex-col items-center text-center">
      <div className="relative w-64 h-64 mb-8">
        <div className="absolute inset-0 flex items-center justify-center">
          <Film className="h-32 w-32 text-primary opacity-80" />
      ...
      <p className="text-xl text-muted-foreground mb-8 max-w-md">
        We couldn't find the movie you're looking for. It might have been removed or the ID is incorrect.
      </p>

      <div className="max-w-md w-full mb-10">
        <p className="mb-2 text-muted-foreground">Try searching for a movie:</p>
        <SearchBar />
      </div>
      ...
      <div className="mt-16 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 max-w-4xl">
        <MovieSuggestion id="299534" title="Avengers: Endgame" />
        <MovieSuggestion id="299536" title="Avengers: Infinity War" />
        <MovieSuggestion id="24428" title="The Avengers" />
        <MovieSuggestion id="157336" title="Interstellar" />
        <MovieSuggestion id="238" title="The Godfather" />
      </div>
    </main>
  )
}
