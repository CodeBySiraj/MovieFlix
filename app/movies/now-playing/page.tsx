...
async function getNowPlayingMovies(page = "1") {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.TMDB_API_KEY}&language=en-US&page=${page}&region=US`,
...
async function MoviesContent({ page }: { page: string }) {
  const [moviesData, genresData] = await Promise.all([getNowPlayingMovies(page), getMovieGenres()])
  ...
  return (
    <div>
      ...
      <Pagination
        ...
        baseUrl={`/movies/now-playing?page=`}
      />
    </div>
  )
}
...
export default function NowPlayingMoviesPage({
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <p className="text-muted-foreground mb-8">Movies currently playing in theaters</p>
